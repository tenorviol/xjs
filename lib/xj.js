		...
		if (node.type == 'tag') {
			for (var i in node.attributes) {
				if (typeof node.attributes[i] == 'object') {
					node.attributes[i] = new Script(node.attributes[i].text);
				}
			}
			...
			var script = new Script(node.text);
			renderable.push(script);
		} else {
...
Tag.prototype = {
	render: function(callback, context) {
		...
		if (this.attributes) {
			result += renderAttributes(this.attributes, context);
		}
		result += '>';
...
function renderAttributes(attributes, context) {
	...
	for (var name in attributes) {
		...
		if (attribute.run) {
			var result = attribute.run(context);
			if (result) {
				render += ' ' + name + '="' + escapeHtml(result) + '"';
			}
		} else {
...
function renderChildren(callback, context) {
	var result = '';
